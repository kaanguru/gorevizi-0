...
import { Badge, Dialog, useThemeMode, Card, Button, CheckBox, Divider } from '@rneui/themed';
      ...
      <View className="flex-1 ">
        ...
        <View className="-mt-12 me-5 justify-end">
          <Pressable className="px-5" onPress={() => router.push(`/(tasks)/edit/${taskID}`)}>
            <FontAwesome6 name="pencil" size={22} color={mode === 'dark' ? '#FFFAEB' : '#051824'} />
          </Pressable>
          <Pressable className="me-1" onPress={() => setShowAlertDialog(true)}>
            <Ionicons name="trash-bin" size={24} color={mode === 'dark' ? '#FF006E' : '#BE004E'} />
          </Pressable>
          <Dialog isVisible={showAlertDialog} onBackdropPress={handleClose}>
            ...
            <Dialog.Button
              ...
              className="px-[30px]"
            />
            <Dialog.Button title="Cancel" onPress={handleClose} size="sm" className="px-[30px]" />
          </Dialog>
        </View>
        <Card>
          ...
          <View className="mx-auto my-5 w-full items-center justify-center px-4">
            {toggleCompleteIsPending ? (
              ...
              <CheckBox
                ...
                checkedIcon={<FontAwesome6 name="check" size={16} color="#8AC926" />}
              />
            )}
          </View>
          {!task.repeat_period && (
